...
const isSkip = (me) => (p) =>
  Array.isArray(me.skip) ? includes(me.skip, p.name) : false
...
const pickRecipients = (participants, runCount = 1) => {
  if (runCount > MAX_RUNS)
    throw new Error(
      `Picking recipients failed. It exceeded max run count of ${MAX_RUNS} which probably means your participants config array contains impossible skip constraints.`
    )

...
module.exports = (config, sendForReal, cb) => {
  ...
  const messages = getMessages({message, twilio: twilioConfig, participants: pickedParticipants})

  const formatResults = (res) => sendForReal ? map(res, (m) => ({sid: m.sid, to: m.to})) : res
